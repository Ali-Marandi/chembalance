...
def convert_mass_to_mass(
    ...
    *,
    ...
    # Preserve unrounded values in the model; round only the explanatory text.
    source_mass_display = _display(source_quantity.grams, 3)
    source_molar_mass_display = _display(source_molar_mass, 3)
    target_molar_mass_display = _display(target_molar_mass, 3)
    source_moles_display = _display(source_moles, 6)
    target_moles_display = _display(target_moles, 6)
    target_mass_display = _display(target_mass_g, 3)
    steps = (
        f"{source_mass_display} g {source.display_formula} ÷ {source_molar_mass_display} g/mol = {source_moles_display} mol",
        f"{source_moles_display} mol {source.display_formula} × {target_coefficient}/{source_coefficient} = {target_moles_display} mol {target.display_formula}",
        f"{target_moles_display} mol {target.display_formula} × {target_molar_mass_display} g/mol = {target_mass_display} g",
    )
